...
8. **User Story:** As a library consumer, I want to express advanced Cypher path patterns (quantified paths, shortest paths, path selectors), so that I can match complex graph structures.
   - Acceptance Criteria:
...
9. **User Story:** As a library consumer, I want to construct `LOAD CSV` queries, so that I can express data import operations.
   - Acceptance Criteria:
...
10. **User Story:** As a library consumer, I want to specify target graphs for queries, so that I can work with composite databases.
    - Acceptance Criteria:
...
11. **User Story:** As a library consumer, I want to add query hints to my statements, so that I can influence the query planner.
    - Acceptance Criteria:
...
12. **User Story:** As a library consumer, I want a fluent builder API starting from a `Cypher` entry point, so that I can construct queries in a readable, chainable style.
    - Acceptance Criteria:
...
13. **User Story:** As a library consumer, I want access to Cypher's built-in functions, so that I can use them within expressions.
    - Acceptance Criteria:
...
14. **User Story:** As a library consumer, I want to render a `Statement` into a Cypher string, so that I can send it to a Neo4j driver or inspect it.
    - Acceptance Criteria:
...
15. **User Story:** As a library consumer, I want to use advanced Cypher expressions (CASE, list comprehensions, pattern comprehensions, map projections, subquery expressions), so that I can express complex logic.
    - Acceptance Criteria:
...
16. **User Story:** As a library consumer, I want to inspect the metadata of a built statement (labels, relationship types, properties, parameters used), so that I can perform analysis or validation before execution.
    - Acceptance Criteria:
...
17. **User Story:** As a library consumer, I want to use Cypher 25 clauses (`FINISH`, `FILTER`, `LET`, `WHEN`, `NEXT`), so that I can take advantage of newer Cypher language features.
    - Acceptance Criteria:
...
18. **User Story:** As a library maintainer, I want the Rust test suite to be derived from the Java Cypher-DSL test suite, so that feature parity is systematically validated.
    - Acceptance Criteria:
...
19. **User Story:** As a Rust developer, I want the library to follow Rust idioms and conventions, so that it feels natural to use alongside other Rust crates.
    - Acceptance Criteria:
...
20. **User Story:** As a library consumer, I want to optionally parse Cypher query strings into the AST, so that I can analyze, validate, or transform existing queries.
    - Acceptance Criteria:
...
21. **User Story:** As a library consumer, I want to construct Neo4j administration commands (index/constraint management, SHOW commands, transaction management), so that I can manage database schema and operations programmatically.
    - Acceptance Criteria:
